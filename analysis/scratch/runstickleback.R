...
split_data <- function(sensors, events) {
  ...
  deployids <- sample(unique(events$deployid), size = 5, replace = FALSE)
  get_start <- function(deployid) {
    e_t <- events$datetime[events$deployid == deployid]
    # Assuming a max 40 s window, a 21 s buffer ensures at least one event
    # falls within the sample window
    result <- sample(e_t, 1) - runif(1, 21, 2 * 3600 - 21)
  }
  ...
  train_events <- map_dfr(deployids, function(id) {
    ...
    events %>%
      filter(deployid == id,
             ...
             datetime > start_time + 20,
             datetime < start_time + 3600 * 2 - 20) %>%
...
create_features <- function(sensors) {
  roll_fn <- function(fn) {
    function(x) fn(x, n = 400, fill = NA, align = "center")
  }
  roll_fn2 <- function(fn) {
    function(x) rollapply(x, width = 400, FUN = fn, fill = NA, align = "center")
  }
  stat_fns <- list(
...
train_randomforest <- function(sensors, events) {
  ...
  ranger(rf_form, features, num.trees = 200)
}
...
test_randomforest<- function(m, sensors, events) {
  ...
  list(f1 = with(o, f1(tp, fp, fn)),
       delta_r = with(o, delta_r(tp, fp, fn, durations)))
}

run_cv <- function(sensors, events) {
